...
const placeOrder = async (req, res) => {
  try {
    ...
    const cart = await Cart.findOne({ userId }).populate({
      ...
      populate: {
        path: 'brand category offer', // Populate brand and category here
        model: 'Brand' // Assuming you're using the same name for Brand model
      }
    });
    ...
    for (const item of cart.items) {
      const product = await Product.findById(item.productId).populate('brand').populate('category');
...
const loadMenPage = async (req, res) => {
  try {
    ...
    const productsWithDiscounts = products.map(product => {
      ...
      // Check for active offers and find the highest discount
      if (product.offer && product.offer.length > 0) {
        const highestDiscountOffer = product.offer
          .filter(offer => offer.status === 'active')
          .reduce((max, offer) => offer.discount > max ? offer.discount : max, 0);
